...
function QuietHeader() {
  ...
  return (
    <header className={`site-header quiet-header ${mobileOpen ? "is-menu-open" : ""}`}>
      <a className="brand" href="#top" aria-label="Axon home">
        <AxonMark />
        <span>axon</span>
      </a>
      <nav className={`header-nav ${mobileOpen ? "is-open" : ""}`} aria-label="Primary navigation">
        <a href="#model" onClick={() => setMobileOpen(false)}>How it works</a>
        <a href="#maintainers" onClick={() => setMobileOpen(false)}>For maintainers</a>
        <a href="/dashboard" onClick={() => setMobileOpen(false)}>Open app <ArrowUpRight size={13} /></a>
        <a href="/docs" onClick={() => setMobileOpen(false)}>Docs <ArrowUpRight size={13} /></a>
        <a className="mobile-only-cta" href="/register" onClick={() => setMobileOpen(false)}>Register a protocol <ArrowUpRight size={14} /></a>
      </nav>
      <button className="header-cta" onClick={() => scrollToSection("maintainers")}>Register a protocol <ArrowUpRight size={16} /></button>
      <button className="mobile-menu" aria-label="Toggle navigation" onClick={() => setMobileOpen(!mobileOpen)}>
